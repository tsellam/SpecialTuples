...
then which variable should the users inspect? Harder still, which
\emph{combination} of variables should they inspect? We cannot assume that
...
\textbf{How can we efficiently choose columns to characterize  our explorers'
tuples?}
%\end{framed}

One way to solve this problem is \emph{subspace search}~\cite{Agrawal1998,
...
database. A fundamental assumption behind our work is that the user is
...
To visualize the subspaces, our analyst may use tables or charts, as in
...
function returns 0 if the tuples come from the same distribution, it grows
...
\begin{table*}[t!]
  ...
  \rowcolors{2}{gray!25}{white}
    ...
    \rowcolor{gray!50}
      ...
      Known as Glass' $\Delta$~\cite{cohen1977statistical}\\
      Stand. Dev.& Contin.  & - &
...
dissimilarity and diversity: a high value enforces that the view are
...
\begin{figure}
  \centering
  \includegraphics[width=0.66\columnwidth]{Figures/Zig-Dissim}
...
We chose these indicators because they have intuitive
...
In principle, we could test differences in spaces with more than two
...
The first step is straightforward, but it is also by far the most time
...
of $\binom{M}{D}$ possible view combinations, which would lead to an
...
problem: permutation testing. This method works with the Zig-Dissimilarity, but
it can also handle other divergence measures. It consists in repeatedly
...
To aggregate the tests, we report the minimum observed confidence, a purposely
...
associated with each column of the database. It then ge\-nerates a short
...
order to capture a wide range of effects. We do not have to use
...
three aspects of our system: the \emph{quality} of the views, their
...
\texttt{4S} is unsupervised, it has therefore no way to detect which subspaces
...
\begin{figure}[t!]
  \centering
  \includegraphics[height=3.25cm]{Plots/Synth-Dedup}
