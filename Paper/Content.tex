...
We can map this task to our problem by treating the user's selection like a
column to be predicted. The main difference is that feature selection seeks
statistical accuracy, while we target interpretation. This, most feature
selection algorithms seek one optimal set of variables, while we seek several
small sets of variables. Also, most algorithms optimize class separability only,
...
\textbf{Data Exploration} Recent work in data exploration have tackled
different but related problems. SeeDB~\cite{vartak2014seedb} also recommends
visualizations, but it focuses on choosing \texttt{GROUP BY} aggregates in a
data warehouse scenario. Lloyd et al.'s automated
