...
show every possible aspect of the data, thus they cannot cope with the scale of
the datasets we target.  Dimensionality reduction algorithms such as PCA or
...
characterization}. The main idea is to show how the selected tuples
\emph{differ} from the rest of the database. To do so, we identify sets of
columns on which the statistical distribution of these tuples is unusual.  We materialize
this idea with Ziggy, a \emph{subset characterization engine}. Ziggy has two
objectives: 1) It detects \emph{interpretable, non-redundant and statistically
robust} sets of such columns. It does so quickly: thanks to aggressive
optimizations, Ziggy can process 10,000s tuples on dozens of variables within a
second. It can cope with both numerical and nominal data, inclu\-ding missing
values. 2) Ziggy can \emph{explain} its findings, with visualizations but also
\emph{natural language}.  Thus, plugged on top of a database engine, Ziggy can
veritably simulate a conversation: users issue queries, and Ziggy replies in
plain English. While previous studies have described ways to \emph{query}
databases in natural language~\cite{li2014constructing}, little to none of them
have focused on descri\-bing the results. To summarize:
\begin{itemize0}
...
real and synthetic data. Finally, we present related work and conclude in
...
\begin{figure}
  \centering
  \includegraphics[width=0.8\columnwidth]{Figures/Example}
...
compared to the rest of the database. We assume that our user is interested in
the differences between her selection and the remaining tuples, not the
similarities.  This is a strong hypothesis, but recent work has shown that it
covers a wide range of use cases~\cite{vartak2015see}.  Se\-cond, we want to
describe why we picked those columns: how is the selection special?
...
Once we detected the subspaces, we can present them with visualizations as in
Figure~\ref{pic:example}. The merits of this approach are hardly disputable, in
particular compared to tables. And yet, analyzing charts takes time and
training. Besides, not all effects are perceptible, in particular when the
views contain more than two dimensions. Therefore, we propose to complete the
visualizations with natural language descri\-ptions, such as the following:
\begin{quotation}
...
Measuring redundancy between columns is not straightforward, as this notion is
...
has little practical effect on Ziggy's accuracy.
...
\begin{figure}
  \centering
  \includegraphics[width=0.7\columnwidth]{Figures/Redundancy}
...
\textbf{Size of the Views D.} In our implementation, we set this parameter in
...
\texttt{Ziggy-Hard},  in different orders (although \texttt{Crime} is a
