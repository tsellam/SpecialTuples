...
insights.  To help them, several authors have proposed data exploration
systems~\cite{abouzied2012dataplay, dimitriadou2014explore, liarou2014dbtouch,
sellam2013meet}. These systems offer some textual or graphical interface,
through which explorers can create, visualize and modify selections of tuples
quickly. Thus, users are engaged in a tight trial-and-error loop, through which
they discover their datasets. 
...
readily replace the output of a data exploration system. To summarize: \begin{itemize0}
...
\begin{figure}
  \centering
  \includegraphics[width=0.65\columnwidth]{Figures/Zig-Dissim}
...
Figure~\ref{pic:zigdis}. We obtain a vector of $V$ Zig-Components. We could add
more components by measuring other properties, such as the difference between
the variances.  Finally, we aggregate these components into one
composite indicator: this is is the Zig-Dissimilarity.
...
$\sqrt{\chi^2/(N(\min(|\bf{d}_t|,|\bf{d}_{\nott}|) - 1))}$, where $\chi^2$ is Pearson's
$\chi^2$ statistic, and $|\bf{d}_t|,|\bf{d}_{\nott}|$ are the number of distinct
values in $\bf{d}_t$ and $\bf{d}_{\nott}$ respectively.
...
Table~\ref{tab:dissim} shows that we report univariate and bivariate
statistics.  In principle, we could test spaces with more dimensions.  We chose
...
outputs. We will show in Section~\ref{sec:experiments} that this
restriction has little practical effect on the Ziggy's views.
...
the former while constraining the latter, as exposed in Equation~\ref{prob2}.
...
Algorithm~\ref{algo:view_search}. Note that initializing the views is not
trivial: first Ziggy inserts a pair of columns, then it adds the candidates 
one at a time.
...
its competitors also test higher level dependencies.
...
duan2014mining,knorr1999finding,zhang2006detecting}. This task directly
inspired our work, but its objectives are different. Outlier description
describes single objects, while we describe sets of tuples.  It focuses on
distance-based outliers, while we focus on probability distributions,
regardless of how central or isolated the user's selection is.  Each of these
work focus on one data type each, and little of them  mention redundancy (an
exception is~\cite{duan2014mining}, which seeks closed sets). None of these
works discuss how to report results.
...
small sets of variables. Also, they optimize class separability only,
...
different but related problems. SeeDB~\cite{vartak2015see} also recommends
visualizations, but it focuses on \texttt{GROUP BY} aggregates in a data
warehouse scenario. Lloyd et al.'s automated statistician~\cite{Lloyd2014ABCD}
...
We are genuinely excited by the perspectives offered by this study. In the
future, we will study how our system can adapt to our user's preferences. We
